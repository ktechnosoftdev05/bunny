...
<div id="kt_header_mobile" class="header-mobile {{ Metronic::printClasses('header-mobile', false) }}" {{ Metronic::printAttrs('header-mobile') }}>
    <div class="mobile-logo">
        <a href="{{ url('/') }}">
            ...
            @php
                $kt_logo_image = 'Splash_Screen_Slider.svg'
            @endphp

            @if (config('layout.aside.self.display') == false)

                @if (config('layout.header.self.theme') === 'light')
                    @php $kt_logo_image = 'Splash_Screen_Slider.svg' @endphp
            ...
            <img alt="{{ config('app.name') }}" src="{{ asset('public/media/logos/'.$kt_logo_image) }}"/>
        </a>
    </div>
    <div class="d-flex align-items-center">
        ...
        <button class="btn btn-hover-text-primary p-0 ml-2" id="kt_header_mobile_topbar_toggle">
            {{ Metronic::getSVG('public/media/svg/icons/General/User.svg', 'svg-icon-xl') }}
        </button>
